...
import httpx
import truststore
from app.gigachat.streaming import collect_stream, token_receiver
...
class GigaChatClient:
    ...
    def chat(
        ...
        try:
            ...
            receiver = token_receiver.get()
            if receiver is not None:
                return collect_stream(self.client, request, receiver)
    ...
    # =========================================================
    # Эмбеддинги (векторная база знаний)
    # =========================================================
    ...
    def embeddings(
        ...
        Используется как офлайн - при сборке индекса базы знаний
        (scripts/build_knowledge_index.py), так и в рантайме - для
        одного короткого запроса на эмбеддинг вопроса пользователя
        (см. app/knowledge/vector_store.py).

        Обработка ошибок зеркалит chat(): вызывающий код (в первую
        очередь VectorKnowledgeBase) не должен знать внутренние типы
        исключений GigaChat SDK/httpx, только наши нормализованные
        исключения.
        """
        ...
        try:
            ...
            logger.warning(
                "GigaChat embeddings timeout after %.1f seconds",
                self.timeout,
            )
            ...
            logger.warning(
                "GigaChat embeddings network error: %s",
                exc,
            )
            ...
            logger.error(
                "GigaChat authentication error during embeddings: %s",
                exc,
            )
            ...
            logger.warning(
                "GigaChat embeddings rate limit: %s",
                exc,
            )
            ...
            logger.warning(
                "GigaChat embeddings server error: %s",
                exc,
            )
            ...
            logger.error(
                "GigaChat embeddings bad request: %s",
                exc,
            )
            ...
            logger.warning(
                "GigaChat embeddings connection error: %s",
                exc,
            )
            ...
            logger.warning(
                "GigaChat embeddings OS/network error: %s",
                exc,
            )
            ...
            logger.exception(
                "Unhandled GigaChat SDK error during embeddings"
            )
            ...
            logger.exception(
                "Unexpected GigaChat embeddings error"
            )
